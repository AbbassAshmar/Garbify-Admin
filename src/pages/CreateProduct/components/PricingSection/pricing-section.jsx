...
export default function PricingSection({formData, setFormData, errors}){
    ...
    return (
        <SectionDefault title={"Pricing"}>
            ...
            <Input label={"sale_start_date"} title={renderColoredLabel('Sale start date')} errors={errors?.messages['sale_start_date']}>
                <TextInputField value={formData.sale_start_date} onChange={e=>handleInputValueChange(e,'sale_start_date')} $error={errors?.messages['sale_start_date']} disabled={formData['sale'] == 0} style={{color:"grey"}} id="sale_start_date" type="date" min="0" />
            </Input>

            <Input label={"sale_end_date"} title={renderColoredLabel('Sale end date','(optional)')} errors={errors?.messages['sale_end_date']}>
                <TextInputField value={formData.sale_end_date} onChange={e=>handleInputValueChange(e,'sale_end_date')} $error={errors?.messages['sale_end_date']} disabled={formData['sale'] == 0} style={{color:"grey"}} id="sale_end_date" type="date" min="0"/>
            </Input>
